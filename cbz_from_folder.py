...
for arg in args:
    ...
    if cbz.exists():
        add_tag(Tag(name="Yellow", color=Color.YELLOW), file=str(src))
        continue
    ...
    if zipfile.is_zipfile(cbz):
        add_tag(Tag(name="Green", color=Color.GREEN), file=str(cbz))
        shutil.rmtree(src)
    else:
        add_tag(Tag(name="Red", color=Color.RED), file=str(cbz))
